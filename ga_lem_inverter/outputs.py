...
def write_summary(context: RunContext) -> Path:
    ...
    lines.extend([
        ...
        f"- 初始地形: `{context.config.get('Model', 'initial_topography', fallback='random')}`",
        f"- flat 初始海拔: `{context.config.get('Model', 'initial_elevation', fallback='0.0')}` m",
        f"- 随机初始地形种子: `{context.config.get('Model', 'initial_topography_seed', fallback='42')}`",
        "",
        "## 指标",
        "",
    ])
