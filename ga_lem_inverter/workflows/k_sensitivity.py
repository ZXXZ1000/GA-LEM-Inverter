...
from ga_lem_inverter.pipeline.array_io import safe_save_array
...
from ga_lem_inverter.pipeline.visualization import plot_comparison
from ga_lem_inverter.pipeline.fitness import terrain_similarity
from ga_lem_inverter.config import get_int_list, get_shape
from ga_lem_inverter.outputs import RunContext, write_metrics
from ga_lem_inverter.workflows.common import run_synthetic_case


def run_k_sensitivity_workflow(config, context: RunContext) -> dict:
    """Run a lightweight scale_factor/K sensitivity experiment."""
    k_values = get_int_list(config, "KSensitivity", "k_values", [4, 8])
    pattern = config.get("KSensitivity", "pattern", fallback=config.get("Synthetic", "pattern", fallback="simple"))
    shape = get_shape(config, "KSensitivity", "shape", get_shape(config, "Synthetic", "shape", (64, 64)))
    logging.info("运行 k_sensitivity 模式: k_values=%s, pattern=%s, shape=%s", k_values, pattern, shape)

    all_metrics = {}
    for scale_factor in k_values:
        config["Optimization"]["scale_factor"] = str(scale_factor)
        prefix = f"k_sensitivity_k{scale_factor}"
    ...
    write_metrics(context, "k_sensitivity_summary.json", all_metrics)
    _plot_k_sensitivity_summary(context, k_values, all_metrics)
    return all_metrics


def _plot_k_sensitivity_summary(context: RunContext, k_values: list[int], metrics: dict) -> None:
    terrain_corr = [metrics.get(f"k_sensitivity_k{k}_terrain_pearson", np.nan) for k in k_values]
    uplift_corr = [metrics.get(f"k_sensitivity_k{k}_uplift_pearson", np.nan) for k in k_values]
    fitness = [metrics.get(f"k_sensitivity_k{k}_best_fitness", np.nan) for k in k_values]

    fig, ax1 = plt.subplots(figsize=(8, 5))
    ax1.plot(k_values, terrain_corr, marker="o", label="Terrain Pearson")
    ax1.plot(k_values, uplift_corr, marker="s", label="Uplift Pearson")
    ax1.set_xlabel("scale_factor / K")
    ax1.set_ylabel("Correlation")
    ax1.grid(True, alpha=0.3)
    ax2 = ax1.twinx()
    ax2.plot(k_values, fitness, marker="^", color="tab:red", label="Best Fitness")
